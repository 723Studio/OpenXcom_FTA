...
Mod::Mod() :
	...
	_ftaGameLength(3), _ironManEnabled(true), _researchTreeDisabled(false),
	...
	_performanceBonusFactor(0.0),  _covertOpsExperienceFactor(100), _covertOpsIntelBonusFactor(100),
	_enableNewResearchSorting(false), _displayCustomCategories(0), _shareAmmoCategories(false), _showDogfightDistanceInKm(false), _showFullNameInAlienInventory(false),
	...
	_defeatScore(0), _defeatFunds(0), _reputationBreakthroughValue(500), _difficultyDemigod(false), _startingTime(6, 1, 1, 1999, 12, 0, 0), _startingDifficulty(0),
	...
	_facilityListOrder(0), _craftListOrder(0), _covertOperationListOrder(0), _itemCategoryListOrder(0), _itemListOrder(0), _armorListOrder(0), _alienRaceListOrder(0),
...
Mod::~Mod()
{
	...
	for (auto& pair : _intelligence)
	{
		delete pair.second;
	}
	for (auto& pair : _prisoners)
	{
		delete pair.second;
	}
	...
	for (auto& pair : _battleScripts)
	{
		for (auto* battleScript : pair.second)
		{
			delete battleScript;
		}
	}
	...
	for (auto& pair : _diplomacyFactions)
	{
		delete pair.second;
	}
	...
	for (auto& pair : _covertOperations)
	{
		delete pair.second;
	}
...
void Mod::loadConstants(const YAML::YamlNodeReader &reader)
{
	...
	reader.tryRead("baseinfoCursor", BASEINFO_CURSOR);
...
void Mod::loadFile(const FileMap::FileRecord &filerec, ModScript &parsers)
{
	...
	for (const auto& ruleReader : iterateRules("intelligence", "name"))
	{
		RuleIntelProject *rule = loadRule(ruleReader, &_intelligence, &_intelligenceIndex, "name", RuleListOrderedFactory<RuleIntelProject>{ _intelligenceListOrder, 100 });
		if (rule != 0)
		{
			rule->load(ruleReader, this);
		}
	}
	for (const auto& ruleReader : iterateRules("prisoners", "type"))
	{
	...
	for (const auto& ruleReader : iterateRules("diplomacyFactions", "name"))
	{
		RuleDiplomacyFaction* rule = loadRule(ruleReader, &_diplomacyFactions, &_diplomacyFactionIndex, "name");
		if (rule != 0)
		{
			rule->load(ruleReader);
		}
	...
	for (const auto& ruleReader : iterateRules("covertOperations", "name"))
	{
		RuleCovertOperation* rule = loadRule(ruleReader, &_covertOperations, &_covertOperationIndex, "name");
		if (rule != 0)
		{
			_covertOperationListOrder += 100;
			rule->load(ruleReader, this, _covertOperationListOrder);
		}
	}
	for (const auto& ruleReader : iterateRules("objects", "type"))
	{
		RuleObject* rule = loadRule(ruleReader, &_objects, &_objectIndex, "type");
		if (rule != 0)
		{
			rule->load(ruleReader);
		}
	...
	for (const auto& ruleReader : iterateRulesSpecific("battleScripts"))
	{
		std::string type = ruleReader["type"].readVal<std::string>();
		if (ruleReader["delete"])
		{
		...
		for (const auto& commandsReader : ruleReader["commands"].children())
		{
			...
			battleScript->load(commandsReader);
	...
	reader.tryRead("ufopaediaUnlockResearch", _ufopaediaUnlockResearch);
	reader.tryRead("alienTechUnlockResearch", _alienTechUnlockResearch);
	reader.tryRead("xenologyUnlockResearch", _xenologyUnlockResearch);
	reader.tryRead("craftsBeamUnlockResearch", _craftsBeamUnlockResearch);
	reader.tryRead("craftSynapseUnlockResearch", _craftSynapseUnlockResearch);
	reader.tryRead("craftGravControlUnlockResearch", _craftGravControlUnlockResearch);
	...
	reader.tryRead("baseConstructionUnlockResearch", _baseConstructionUnlockResearch); // Duplication in FtA, works a bit difference
	...
	reader.tryRead("researchTrainingFactor", _researchTrainingFactor);
	reader.tryRead("engineeringTrainingFactor", _engineeringTrainingFactor);
	reader.tryRead("intelTrainingFactor", _intelTrainingFactor);
	reader.tryRead("researchSpeedFactor", _researchSpeedFactor);
	reader.tryRead("engineeringSpeedFactor", _engineeringSpeedFactor);
	reader.tryRead("intelSpeedFactor", _intelSpeedFactor);
	reader.tryRead("prisonerActionsSpeedFactor", _prisonerActionsSpeedFactor);
	...
	reader.tryRead("covertOpsExperienceFactor", _covertOpsExperienceFactor);
	reader.tryRead("covertOpsIntelBonusFactor", _covertOpsIntelBonusFactor);
	reader.tryRead("visibilityStatsMod", _visibilityStatsMod);
	...
	reader.tryRead("loyaltyRatings", _loyaltyRatings);
	reader.tryRead("reputationLevels", _reputationLevels);
...
void Mod::sortLists()
{
	...
	sortIndex(_intelligenceIndex, _intelligence, compareRule<RuleIntelProject>(this));
