...
AlienDeployment::AlienDeployment(const std::string &type) :
	...
	_shade(-1), _minShade(-1), _maxShade(-1), _finalDestination(false), _isAlienBase(false), _isHidden(false), _isHiddenAlienBase(false), _isHiddentXcomBase(false), _fakeUnderwaterSpawnChance(0),
...
void AlienDeployment::load(const YAML::YamlNodeReader& node, Mod *mod)
{
	...
	reader.tryRead("alternativeDeployment", _alternativeDeployment);
	reader.tryRead("alternativeDeploymentResearch", _alternativeDeploymentResearch);
	...
	reader.tryRead("battleScript", _battleScript);
	reader.tryRead("mapScripts", _mapScripts);
	reader.tryRead("undercoverArmors", _undercoverArmors);
