...
RuleSoldierTransformation::RuleSoldierTransformation(const std::string &name)
	: _name(name),
	  ...
	  _showMinMax(false), _lowerBoundAtMinStats(true), _upperBoundAtMaxStats(false), _upperBoundAtStatCaps(false), _upperBoundType(0), _addRole(ROLE_NONE), _forbiddentRole(ROLE_NONE),
	  _reset(false)
{
}
...
void RuleSoldierTransformation::load(const YAML::Node &node, Mod* mod, int listOrder)
{
	...
	_forbiddentRole = (SoldierRole)node["forbiddentRole"].as<int>(_forbiddentRole);
