	...
	if (_spent > _cost + (_rolls * getRules()->getCostIncrease()))
	{
		...
		if (!rolledStages.empty())
		{
			...
			if (!pickedStage->getEventScripts().empty())
			{
				...
				game->getGeoscapeState()->eventScriptProcessor(pickedStage->getEventScripts(), OTHER_SCRIPT);
			}
			...
			if (!pickedStage->getSpawnedMission().empty())
			{
				...
				game->getGeoscapeState()->spawnAlienMission(pickedStage->getSpawnedMission(), globe, _base);
			}
